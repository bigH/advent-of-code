...
fn main() {
    ...
    let (mut max_key, mut max_val) = (String::from("a"), 0);
    ...
    for inst in instructions {
        ...
        if inst.test_op.apply(register_value, inst.test_value) {
            let acted_value: &mut i32 = registers.entry(inst.acted_register.clone()).or_insert(0);
            inst.action.apply(acted_value, inst.amount);
            if *acted_value > max_val {
                max_key = inst.acted_register.clone();
                max_val = acted_value.clone();
            }
        }
    }
