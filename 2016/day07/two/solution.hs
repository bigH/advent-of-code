...
tokens str = tokenRight allTokens ([], [])
  where allTokens = concat $ map (split "[") $ split "]" str
        tokenLeft [] (a, b) = (a, b)
        tokenLeft (x:xs) (a, b) = tokenRight xs (a ++ [x], b)
        tokenRight [] (a, b) = (a, b)
        tokenRight (x:xs) (a, b) = tokenLeft xs (a, b ++ [x])

checkTuple :: (a -> c, b -> c) -> (c -> c -> Bool) -> (a, b) -> Bool
checkTuple (f, g) c (a, b) = c (f a) (g b)
...
supportsSSL = doesSupport . tokens
  where doesSupport = checkTuple (extractAba, map reverseAba . extractAba) ((not . null) `chainer` intersect)
        extractAba = map head . group . sort . concat . map getAba

getAba :: String -> [String]
getAba = filter isAba . sliding 3

reverseAba :: String -> String
reverseAba  (a:b:c:[])
  | (a == c) && (a /= b) = b:a:b:[]
  | otherwise            = error "Not an ABA"

isAba :: String -> Bool
isAba (a:b:c:[]) = (a == c) && (a /= b)
isAba _            = False

main = do putStrLn "Number with SSL Support:"
