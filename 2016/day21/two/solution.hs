...
run s (RotateLeft       x) = drop y s ++ take y s
                               where y = (len - x) `rem` len
                                     len = length s
run s (RotateOnLetter   x) = run s command
                               where location = fromJust $ findIndex (== x) s
                                     command = if odd location then RotateRight (location `quot` 2 + 1)
...
main = do input <- readFile "../input"
          ...
          print $ foldl run seed $ reverse $ map parse $ lines input
