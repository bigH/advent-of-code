...
nexts visited path = map (: path) newNodes
...
pathsFrom visited pathsSoFar i =
  if nonEmpty reached
    ...
    else ([visitedMsg, pathsMsg, prunedMsg] ++ messages, final)
      where (reached, rest) = partition (goal . head) pathsSoFar
            newPathsSoFar = nextsForAll visited pathsSoFar
            visitedMsg = " ** Visited:           " ++ show (length visited)
            pathsMsg = " ** All Posibilities:  " ++ show (length newPathsSoFar) ++ " paths of length " ++ show (length $ head newPathsSoFar)
            pruned = prune visited newPathsSoFar
            prunedMsg = " ** `pathsFrom` Prune: " ++ show (length pruned) ++ " paths of length " ++ show (length $ head newPathsSoFar)
