...
isIn :: [Char] -> Char -> Bool
isIn set item = elem item set

notIn :: [Char] -> Char -> Bool
notIn set item = not $ isIn set item
...
sector :: [Char] -> Int
sector = asInt . filter (isIn digits)

asInt :: [Char] -> Int
asInt s = read s :: Int

checksumFrom :: [Char] -> [Char]
checksumFrom = reverse . takeWhile (isIn alphas) . reverse . clean

tuplize :: ((a -> b), (a -> c)) -> a -> (b, c)
tuplize (f, g) x = (f x, g x)

frequencies :: [Char] -> [(Char, Int)]
frequencies = map (tuplize (head, length)) . group . sort . letters

doChecksum :: [Char] -> [Char]
doChecksum = take 5 . reverse . map fst . sortBy frequencyOrder . frequencies

frequencyOrder :: (Char, Int) -> (Char, Int) -> Ordering
frequencyOrder (a, b) (c, d)
  | b < d     = LT
  | b > d     = GT
  | a < c     = GT
  | a > c     = LT
...
initialLetters :: [Char] -> [Char]
initialLetters = takeWhile $ isIn alphas

letters :: [Char] -> [Char]
letters = initialLetters . clean

clean :: [Char] -> [Char]
clean = filter $ notIn specials

valid :: [Char] -> Bool
valid room = checksumFrom room == doChecksum room

decoder :: Int -> [Char] -> [Char]
decoder i = map $ decode i

decode :: Int -> Char -> Char
decode i c
  | c > 'z'         = c
  | c < 'a'         = c
  | otherwise       = applyDecode i c

applyDecode :: Int -> Char -> Char
applyDecode i c
  | c > 'z'         = applyDecode i $ chr $ ord c - 26
  | c < 'a'         = applyDecode i $ chr $ ord c + 26
  | i == 0          = c
  | i < 0           = applyDecode (i + 26) c
  | i >= 26         = applyDecode (i - 26) c
  | otherwise       = applyDecode (i - 1) (succ c)

hasPresents :: [Char] -> Bool
hasPresents room = isInfixOf "presents" decoded
  where roomDecoder = decoder $ sector room
        decoded = roomDecoder $ letters room

decodeRoom :: [Char] -> [Char]
decodeRoom room = decoder (sector room) room

main = do putStrLn "Sum of Valid Room Sectors:"
          print $ foldr (+) 0 $ map sector $ filter valid rooms
          putStrLn ""
          putStrLn "Room with the `presents`:"
          putStrLn $ unlines $ map decodeRoom $ filter valid rooms
